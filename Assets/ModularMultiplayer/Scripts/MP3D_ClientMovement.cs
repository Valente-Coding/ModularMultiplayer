...
public class MP3D_ClientMovement : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (m_EnableHorizontalMovement)
        {
            ...
            if (m_KeyPressed_A || m_KeyPressed_D)
            {
                ...
                transform.position += transform.right * m_MoveX;
            }
        }
        ...
        if (m_EnableVerticalMovement)
        {
            ...
            {
                m_MoveZ = (m_KeyPressed_W ? l_multiplier : 0) + (m_KeyPressed_S ? -l_multiplier : 0);

                transform.position += transform.right * m_MoveZ;
            }
        }
#else
        if (m_EnableHorizontalMovement)
        ...
        if (m_EnableVerticalMovement)
        {
            ...
            if (m_KeyPressed_W || m_KeyPressed_S)
            {
                ...
                transform.position += transform.forward * m_MoveZ;
            }
        }
#endif
    }
